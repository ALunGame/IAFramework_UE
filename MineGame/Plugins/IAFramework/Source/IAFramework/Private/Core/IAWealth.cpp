...
struct ObjectSingleLoadNode
{
	...
	ObjectSingleLoadNode(TSharedPtr<FStreamableHandle> InWealthHandle, FObjectWealthEntry* InWealthEntry,
	                     FName InObjectName, FName InFunName)
	{
...
struct ObjectKindLoadNode
{
	...
	FName FunName;
	//构造函数
	ObjectKindLoadNode(TSharedPtr<FStreamableHandle> InWealthHandle, TArray<FObjectWealthEntry*>& InUnLoadWealthEntry,
	                   TArray<FObjectWealthEntry*>& InLoadWealthEntry, FName InObjectName, FName InFunName)
	{
		WealthHandle = InWealthHandle;
...
struct ClassSingleLoadNode
{
	...
	ClassSingleLoadNode(TSharedPtr<FStreamableHandle> InWealthHandle, FClassWealthEntry* InWealthEntry,
	                    FName InObjectName, FName InFunName, FTransform InSpawnTransform)
	{
		WealthHandle = InWealthHandle;
		WealthEntry = InWealthEntry;
		ObjectName = InObjectName;
...
struct ClassKindLoadNode
{
	...
	ClassKindLoadNode(TSharedPtr<FStreamableHandle> InWealthHandle, TArray<FClassWealthEntry*>& InUnLoadWealthEntry,
	                  TArray<FClassWealthEntry*>& InLoadWealthEntry, FName InObjectName, FName InFunName)
	{
		WealthHandle = InWealthHandle;
		UnLoadWealthEntry = InUnLoadWealthEntry;
		LoadWealthEntry = InLoadWealthEntry;
	...
	ClassKindLoadNode(TSharedPtr<FStreamableHandle> InWealthHandle, TArray<FClassWealthEntry*>& InUnLoadWealthEntry,
	                  TArray<FClassWealthEntry*>& InLoadWealthEntry, FName InObjectName, FName InFunName,
	                  TArray<FTransform> InSpawnTransforms)
	{
		WealthHandle = InWealthHandle;
		UnLoadWealthEntry = InUnLoadWealthEntry;
		LoadWealthEntry = InLoadWealthEntry;
...
struct ClassMultiLoadNode
{
	...
	TArray<UUserWidget*> WidgetGroup;
	//构造函数, 未加载时使用
	ClassMultiLoadNode(TSharedPtr<FStreamableHandle> InWealthHandle, FClassWealthEntry* InWealthEntry, int32 InAmount,
	                   FName InObjectName, FName InFunName, TArray<FTransform> InSpawnTransforms)
	{
		WealthHandle = InWealthHandle;
		WealthEntry = InWealthEntry;
		Amount = InAmount;
	...
	//构造函数, 已加载时使用
	ClassMultiLoadNode(FClassWealthEntry* InWealthEntry, int32 InAmount, FName InObjectName, FName InFunName,
	                   TArray<FTransform> InSpawnTransforms)
	{
		WealthEntry = InWealthEntry;
		Amount = InAmount;
...
void UIAWealth::WealthBeginPlay()
{
	//遍历自动生成对象
	for (int i = 0; i < WealthData.Num(); ++i)
	{
		...
		for (int j = 0; j < WealthData[i]->AutoObjectData.Num(); ++j)
		{
			...
			NewObj->AddToRoot();
			IIAOO* InstPtr = Cast<IIAOO>(NewObj);
			if (InstPtr)
			{
				InstPtr->RegisterToModule(
					WealthData[i]->ModuleType == EGameModule::Null ? IModule->ModuleType : WealthData[i]->ModuleType,
		...
		{
			AActor* NewAct = GetIAWorld()->SpawnActor<AActor>(WealthData[i]->AutoActorData[j].WealthClass, WealthData[i]->AutoActorData[j].Transform);
			IIAOO* InstPtr = Cast<IIAOO>(NewAct);
			if (InstPtr)
			{
				InstPtr->RegisterToModule(
					WealthData[i]->ModuleType == EGameModule::Null ? IModule->ModuleType : WealthData[i]->ModuleType,
		...
		{
			UUserWidget* NewWidget = CreateWidget<UUserWidget>(GetIAWorld(), WealthData[i]->AutoWidgetData[j].WealthClass);
			//避免回收
			GCWidgetGroup.Push(NewWidget);
			IIAOO* InstPtr = Cast<IIAOO>(NewWidget);
			if (InstPtr)
			{
				InstPtr->RegisterToModule(
					WealthData[i]->ModuleType == EGameModule::Null ? IModule->ModuleType : WealthData[i]->ModuleType,
		...
		for (int j = 0; j < WealthData[i]->ObjectWealthData.Num(); ++j)
			WealthData[i]->ObjectWealthData[j].WealthObject = nullptr;
		for (int j = 0; j < WealthData[i]->ClassWealthData.Num(); ++j)
			WealthData[i]->ClassWealthData[j].WealthClass = nullptr;
#endif
	}
}

void UIAWealth::WealthTick(float DeltaSeconds)
{
...
void UIAWealth::AssignData(TArray<UWealthData*>& InWealthData)
{
	WealthData = InWealthData;
}

FWealthURL* UIAWealth::GainWealthURL(FName WealthName)
{
	for (int i = 0; i < WealthData.Num(); ++i)
...
void UIAWealth::GainWealthURL(FName WealthKind, TArray<FWealthURL*>& OutURL)
{
	for (int i = 0; i < WealthData.Num(); ++i)
		for (int j = 0; j < WealthData[i]->WealthURL.Num(); ++j)
...
void UIAWealth::LoadObjectWealth(FName WealthName, FName ObjectName, FName FunName)
{
	//获取资源结构体
	...
	if (!WealthEntry)
	{
		IA::Debug() << ObjectName << " Get Null Wealth : " << WealthName << IA::Endl();
		return;
	}
	//如果资源不可用
	if (!WealthEntry->WealthPath.IsValid())
	{
		IA::Debug() << ObjectName << " Get UnValid Wealth : " << WealthName << IA::Endl();
		return;
	}
	//如果资源已经加载
...
void UIAWealth::LoadObjectWealthKind(FName WealthKind, FName ObjectName, FName FunName)
{
	TArray<FObjectWealthEntry*> WealthEntryGroup = GetObjectKindEntry(WealthKind);
	//如果数量为0
	if (WealthEntryGroup.Num() == 0)
	{
		IA::Debug() << ObjectName << " Get Null WealthKind : " << WealthKind << IA::Endl();
		return;
	}
	//判断资源可用性
	for (int i = 0; i < WealthEntryGroup.Num(); ++i)
	{
		...
		{
			IA::Debug() << ObjectName << " Get Not Valid in Kind : " << WealthKind << " For Name : " << WealthEntryGroup[i]->WealthName << IA::Endl();
			return;
		}
	}
	//还没有加载的资源
...
void UIAWealth::LoadClassWealth(FName WealthName, FName ObjectName, FName FunName)
{
	//获取资源结构体
...
void UIAWealth::LoadClassWealthKind(FName WealthKind, FName ObjectName, FName FunName)
{
...
void UIAWealth::BuildSingleClassWealth(EWealthType WealthType, FName WealthName, FName ObjectName, FName FunName,
	FTransform SpawnTransform)
{
	//获取对应的资源结构体
	FClassWealthEntry* WealthEntry = GetClassSingleEntry(WealthName);
	...
	{
		IA::Debug() << ObjectName << " Get Error Type : " << WealthName << IA::Endl();
		return;
	}
	//如果资源已经加载
	if (WealthEntry->WealthClass)
...
void UIAWealth::BuildKindClassWealth(EWealthType WealthType, FName WealthKind, FName ObjectName, FName FunName,
	TArray<FTransform> SpawnTransforms)
{
	...
	for (int i = 0; i < WealthEntrytGroup.Num(); ++i)
	{
		...
		{
			IA::Debug() << ObjectName << " Get Not Vaild In Kind : " << WealthKind << " For Name : " << WealthEntrytGroup[i]->WealthName << IA::Endl();
			return;
		}
		//资源类型匹配
		if (WealthEntrytGroup[i]->WealthType != WealthType)
		{
			IA::Debug() << ObjectName << " Get Error Type In Kind : " << WealthKind << " For Name : " << WealthEntrytGroup[i]->WealthName << IA::Endl();
			return;
		}
	}
	...
	{
		IA::Debug() << ObjectName << " Send Error Spawn Count : " << WealthKind << IA::Endl();
		return;
	}
	//未加载的资源链接
	TArray<FClassWealthEntry*> UnLoadWealthEntry;
...
void UIAWealth::BuildMultiClassWealth(EWealthType WealthType, FName WealthName, int32 Amount, FName ObjectName,
	FName FunName, TArray<FTransform> SpawnTransforms)
{
	...
	{
		IA::Debug() << ObjectName << " Send Error Spawn Count : " << WealthName << IA::Endl();
		return;
	}
	//如果已经加载资源
	if (WealthEntry->WealthClass)
...
FObjectWealthEntry* UIAWealth::GetObjectSingleEntry(FName WealthName)
{
	for (int i = 0; i < WealthData.Num(); ++i)
		for (int j = 0; j < WealthData[i]->ObjectWealthData.Num(); ++j)
...
TArray<FObjectWealthEntry*> UIAWealth::GetObjectKindEntry(FName WealthKind)
{
...
FClassWealthEntry* UIAWealth::GetClassSingleEntry(FName WealthName)
{
	for (int i = 0; i < WealthData.Num(); ++i)
		for (int j = 0; j < WealthData[i]->ClassWealthData.Num(); ++j)
...
TArray<FClassWealthEntry*> UIAWealth::GetClassKindEntry(FName WealthKind)
{
...
void UIAWealth::DealObjectSingleLoadStack()
{
	//定义加载完成的序列
...
void UIAWealth::DealObjectKindLoadStack()
{
	//定义加载完成的序列
...
void UIAWealth::DealClassSingleLoadStack()
{
	//定义加载完成的序列
...
void UIAWealth::DealClassKindLoadStack()
{
	//定义已完成加载节点序列
...
void UIAWealth::DealClassMultiLoadStack()
{
	//定义完成的节点
	...
	for (int i = 0; i < ClassMultiLoadStack.Num(); ++i)
	{
		...
		if (ClassMultiLoadStack[i]->WealthEntry->WealthClass)
		{
			...
			else if (ClassMultiLoadStack[i]->WealthEntry->WealthType == EWealthType::Actor)
			{
				...
				//生成对象
				AActor* InstActor = GetIAWorld()->SpawnActor<AActor>(ClassMultiLoadStack[i]->WealthEntry->WealthClass, SpawnTransform);
				//添加参数数组
			...
			{
				UUserWidget* InstWidget = CreateWidget<UUserWidget>(GetIAWorld(), ClassMultiLoadStack[i]->WealthEntry->WealthClass);
				//避免回收
				GCWidgetGroup.Push(InstWidget);
				//添加参数数组
